...
unsigned sim_ooo::get_int_register_tag(unsigned reg) {
  ...
  unsigned char destinationValue[2];
...
void sim_ooo::issue_instruction(){
  ...
  static instruction_t currentInstruction = instr_memory[mem_to_index(pc)];
  static instruction_t previousInstruction;
  static unsigned ROB_ENTRY_NUMBER;
  static res_station_t stationTypeNeeded;
  ...
  if(!rob_full()){
    resStationIndex = reservation_station_full(get_station_type(currentInstruction));
    if(resStationIndex!=UNDEFINED){
      // if not full, add instruction
      stallAtIssue = false;
      ROB_ENTRY_NUMBER = rob_add();
      //output rob index for debugging purposes
      cout<< "Current ROB index: " << ROB_ENTRY_NUMBER << endl;
      //add to reservation station
      reservation_station_add(pc, resStationIndex, currentInstruction, ROB_ENTRY_NUMBER);
      // increment pc
      pc = pc + 4;
      //set previous instruction marker
      previousInstruction = currentInstruction;
      //set current instruction for next iteration of this function
      currentInstruction = instr_memory[mem_to_index(pc)];
...
void sim_ooo::issue_decode(instruction_t thisInstruction){
  /*
   * Arguments for lw and lws:
   * src1??
   * Dest for lw and lws:
   * dest
   * Arguments for ADD, SUB, XOR, AND,
   * src1, src2
   * known operands are written to vj and vk
   * I think vj -> src1 and vk -> src2
   * Dest for ADD, SUB, XOR, AND,
   * dest
   */
...
unsigned sim_ooo::rob_add(){
  //init rob index to 0;
  static unsigned rob_index = 0;
  //init pc to base address
  static unsigned pc = instr_base_address;
  //get instruction
  instruction_t currentInstruction = instr_memory[mem_to_index(pc)];
  //add instruction to rob
  rob.entries[rob_index].pc = pc;
  rob.entries[rob_index].state = ISSUE;
  rob.entries[rob_index].destination = currentInstruction.dest;
  rob.entries[rob_index].empty = false;

  //update register entry to contain 'name' information
  //fp and int reg's are in different arrays
  if(is_fp_instruction(currentInstruction.opcode)){
    float_gp[currentInstruction.dest].name = rob_index;
  }
  else{
    int_gp[currentInstruction.dest].name = rob_index;
  }
  //increment rob index
  rob_index++;
  //increment pc
  pc = pc + 4;
  //rob struct can only hold num_entries, defined at object instantiation
  //if rob index is 6 after incrementing, reset rob index
  //this shouldn't happen though, a logic check should prevent this function from being called if there are no spaces avail
...
unsigned sim_ooo::reservation_station_full(res_station_t thisTypeOfStation){
  for(unsigned i = 0; i< reservation_stations.num_entries; i++){
...
void sim_ooo::reservation_station_add(unsigned thisPC, unsigned thisStationNumber, instruction_t thisInstruction, unsigned thisROBIndex){
  reservation_stations.entries[thisStationNumber].pc = thisPC;
