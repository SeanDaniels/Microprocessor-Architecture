...
int main(int argc, char **argv) {
  ...
  sim_ooo *ooo = new sim_ooo(1024 * 1024, 6, 2, 2, 2, 1);
  ...
  for (i = 0; i < 5; i++)
	ooo->set_fp_register(i, (float)i);

  // initialize data mem ry and prints its content (for the specified address
  // ranges)
  for (i = 0xA000, j = 0; i < 0xA020; i += 4, j += 1)
	ooo->write_memory(i, float2unsigned((float)(j + 1)));
  ...
  cout << "First 20 clock cycles: inspecting the registers at each clock "
		  "cycle..."
	   << endl;
  cout << "===================================================================="
		  "=="
  ...
  cout << "Instruction executed = " << dec << ooo->get_instructions_executed()
	   << endl;
